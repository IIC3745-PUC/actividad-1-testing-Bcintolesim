[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/AG7uuOyW)


# Declaración breve de uso de IA

## Registro de uso
**Herramienta:** Gemini
**Modelo:** Gemini-3
**Parte de la actividad en la que se uso:** Introduccion, codigo
**Prompt:**
```text
En base al archivo entregado, que casos debo cubrir para obtener un 100% en los tests, detalla que parte del codigo se esta revisando en cada caso
```
```text
Como funciona la libreria unittest, como se puede aplicar en este caso, da ejemplos
```
Autocompletacion de codigo con copilot
**Evaluación de la respuesta:** Muy buena
**Justificación:** Me recomendo que partes del codigo debia revisar junto con explicar como funciona la libreria con ejemplos
**¿Requirió trabajo adicional?:** Si
**Trabajo adicional realizado:** La ia se uso para preguntas mas que para completacion de codigo, el trabajo adiccional se baso en los ejemplos de uso de la libreria para aplicarlos al caso de la actividad.
## Reflexión final sobre el uso de IA
La IA fue de suma ayuda para entender la libreria y su correcto uso para esta actividad, junto con explicar como funcionaba el codigo que se brindo. 